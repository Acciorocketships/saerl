...
class SAEModel(Model):
    ...
    def _forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        agent_batch = torch.arange(input.shape[:-1].numel(), device=self.device).repeat_interleave(rel_agent.shape[-2])
        obs_batch = torch.arange(input.shape[:-1].numel(), device=self.device).repeat_interleave(rel_obs.shape[-2])
